...
export default function ActivityAndOffersSection({
  ...
  const { events: traitEvents, isLoading: isTraitEventsLoading } =
    useTraitMarketplaceEvents(tokenId ?? "");
  const { events: chonkEvents, isLoading: isChonkEventsLoading } =
    useChonkMarketplaceEvents(tokenId ?? "");

  const events = type === "trait" ? traitEvents : chonkEvents;
  const isEventsLoading =
    type === "trait" ? isTraitEventsLoading : isChonkEventsLoading;
                ...
                {events.map((event) => (
                  <tr key={event.id} className="border-b border-gray-200">
                    <td className="py-2">
                      {event.type === "offer"
                        ? "Listed"
                        : event.type === "offerToAddress"
                        ? "Private Listed"
                        : event.type === "bought"
                        ? "Purchased from Listing"
                        : event.type === "bid"
                        ? "Bid"
                    ...
                    <td className="py-2">
                      {event.type === "offer"
                        ? "-"
                        : event.type === "offerToAddress"
                        ? truncateEthAddress(event.onlySellTo)
                        : event.type === "bought"
                        ? truncateEthAddress(event.buyer)
                        : event.type === "bid"
                        ? "-"
                        : event.type === "bidWithdrawn"
                        ? "-"
                    ...
                    <tr key={index} className="border-b border-gray-200">
                      ...
                      <td className="py-2">
                        {transfer.transaction_value && (
                          <span className="inline-flex items-center whitespace-nowrap">
                            {transfer.transaction_value / 1e18}{" "}
                            <FaEthereum className="ml-1 text-[1vw]" />
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-2">
                        {address && address === transfer.from_address
                          ? "You"
                          : transfer.from_address
                          ? truncateEthAddress(transfer.from_address)
                          : "-"}
                      </td>
                      <td className="py-2">
                        {address && address === transfer.to_address
                          ? "You"
                          : transfer.to_address
                          ? truncateEthAddress(transfer.to_address)
                          : "-"}
                      </td>
                      <td className="py-2 flex items-center justify-end">
                        ...
                        >
                          {formatDistanceToNow(new Date(transfer.timestamp), {
                            addSuffix: true,
                          })}
                          <IoExitOutline className="ml-2" />
                        </Link>
