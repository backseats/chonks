...
import {
  ...
  NETWORKS,
  readContract,
} from "./common/read-contract";
import { BaseError } from "viem";
import {
  Content,
  ContentType,
  ParsedOutput,
  parseContractFunctionOutput,
} from "./common/parser";
import { colorJsonHtml } from "./common/color-json";
import { Base64 } from "js-base64";
import {
  useDefaultOrSearchParam,
  ParamsMap,
  encodeParams,
} from "./common/search-param-hook";
...
export default function Home() {
  ...
  const [address, setAddress] = useDefaultOrSearchParam(
    "0xb44298c4ef2f474a25a570ce23ece36ecfcd45d3",
    "address",
    paramsMap,
    searchParams
  );
  const [networkId, setNetworkId] = useDefaultOrSearchParam(
    NETWORKS[0].id,
    "network",
    paramsMap,
    searchParams
  ...
  const [hasSecondParam, setHasSecondParam] = useDefaultOrSearchParam(
    false,
    "hasSecondParam",
    paramsMap,
    searchParams
  );
  const [secondParamValue, setSecondParamValue] = useDefaultOrSearchParam(
    "1",
    "secondParamValue",
    paramsMap,
    searchParams
  );
  const [secondParamType, setSecondParamType] = useDefaultOrSearchParam(
    "uint256",
    "secondParamType",
    paramsMap,
    searchParams
  ...
  const [functionName, setFunctionName] = useDefaultOrSearchParam(
    "tokenURI",
    "function",
    paramsMap,
    searchParams
  );
  const [tokenIdType, setTokenIdType] = useDefaultOrSearchParam(
    "uint256",
    "tokenIdType",
    paramsMap,
    searchParams
  );
  const [customNodeUrl, setCustomNodeUrl] = useDefaultOrSearchParam(
    "",
    "customNodeUrl",
    paramsMap,
    searchParams
  );
  const [viewWidth, setViewWidth] = useDefaultOrSearchParam(
    255,
    "viewWidth",
    paramsMap,
    searchParams
  ...
  const [tokenId, setTokenId] = useDefaultOrSearchParam(
    28,
    "tokenId",
    paramsMap,
    searchParams
  );
  const [responsive, setResponsive] = useDefaultOrSearchParam(
    false,
    "responsive",
    paramsMap,
    searchParams
  );
  const [showBorder, setShowBorder] = useDefaultOrSearchParam(
    true,
    "showBorder",
    paramsMap,
    searchParams
  ...
  const handleCustomNodeUrlChange = (e: any) =>
    setCustomNodeUrl(e.target.value);
  const handleNetworkChange = (e: any) => setNetworkId(e.target.value);
  const handleHasSecondParamChange = (e: any) =>
    setHasSecondParam(e.target.checked);
  const handleSecondParamValueChange = (e: any) =>
    setSecondParamValue(e.target.value);
  const handleSecondParamTypeChange = (e: any) =>
    setSecondParamType(e.target.value);
  ...
  const renderContent = (c: Content) => {
    ...
    if (c.contentType == ContentType.SVG) {
      return (
        <img
          src={"data:image/svg+xml;base64," + Base64.encode(c.content)}
          alt=""
        ></img>
      );
    }
  ...
  const invokeContractFunction = async (myTokenId: number) => {
    ...
    try {
      let result = await readContract(
        ...
        tokenIdType,
        customNodeUrl,
        hasSecondParam ? secondParamValue : null,
        hasSecondParam ? secondParamType : null
      );
      ...
      {isHidden && (
        <div className={"text-xs " + (responsive ? "w-full" : PAGE_WIDTH)}>
          <div className="mt-2 flex justify-center items-center text-gray-500">
            <button
              className="bg-gray-300 hover:bg-gray-400 px-1 py-1 rounded"
        ...
        <div className={"text-xs " + (responsive ? "w-full" : PAGE_WIDTH)}>
          <div className="mx-4 mt-1 px-2 py-2 flex flex-col border-2">
            <div className="flex flex-row items-center">
              <span>address</span>
              <input
                className="ml-2 w-full border-2 rounded"
                value={address}
            ...
            <div className="mt-1 flex flex-row items-center justify-between">
              <div className="flex flex-row items-center">
                <span>network</span>
                <select
                  className="mx-2 border-2 rounded"
                  value={networkId}
              ...
              <div className="flex flex-row items-center">
                ...
                <input
                  ...
                  type="checkbox"
                  id="responsive"
                  checked={hasSecondParam}
                  onChange={handleHasSecondParamChange}
                />
              </div>
            </div>
            ...
            {networkId == CUSTOM_NETWORK_ID && (
              <div className="mt-1 flex flex-row items-center">
                ...
                <input
                  className="ml-2 w-full border-2 rounded"
                  value={customNodeUrl}
                  onChange={handleCustomNodeUrlChange}
                />
              </div>
            )}
            ...
            <div className="mt-1 flex flex-row justify-between">
              <div className="flex flex-row items-center">
                <span>function</span>
                <input
                  className="ml-2 border-2 rounded w-48"
                  value={functionName}
              ...
              <div className="ml-4 flex flex-row items-center">
                ...
                <input
                  className="ml-2 border-2 rounded w-16"
                  value={tokenIdType}
                  onChange={handleTokenIdTypeChange}
                />
              </div>
            </div>
            ...
            {hasSecondParam && (
              <div className="mt-1 flex flex-row justify-between">
                <div className="flex flex-row items-center">
                  ...
                  <input
                    className="ml-2 border-2 rounded w-36"
                    value={secondParamValue}
                    onChange={handleSecondParamValueChange}
                  />
                </div>
                <div className="ml-4 flex flex-row items-center">
                  ...
                  <input
                    className="ml-2 border-2 rounded w-16"
                    value={secondParamType}
                    onChange={handleSecondParamTypeChange}
                  />
                </div>
              </div>
            )}
            ...
            <div className="mt-2 flex flex-row items-center justify-between">
              <div className="flex flex-row items-center jutify-start">
                ...
                <span className="ml-4 text-xs">
                  {viewWidth}x{viewWidth} px
                </span>
              </div>

              <div className="flex flex-row items-center jutift-end">
                <button
                  className="ml-3 fill-blue-500 hover:fill-blue-700 py-1 px-1"
                ...
                <button
                  className="ml-2 bg-gray-400 hover:bg-gray-500 text-white font-bold py-1 px-2 rounded"
      ...
      <div className={responsive ? "w-full" : PAGE_WIDTH}>
        <div className="mx-4 mt-4 justify-between items-center flex flex-row">
          ...
          <div className="flex flex-row items-center">
            ...
            <input
              className="w-16 text-right border-2"
              type="number"
              id="tokenId"
              name="tokenId"
              min="0"
        ...
        {(!error || error.length == 0) && (
          <div className={"m-4 flex " + (responsive ? "flex-col" : "flex-row")}>
                  ...
                  <div key={ind}>
                    ...
                    <div
                      className={
                        ...
                        (showBorder ? "border-opacity-100" : "border-opacity-0")
                      }
                      style={{
                        minWidth: responsive ? "100%" : viewWidth + "px",
                      }}
                    >
                      {renderContent(c)}
                    </div>
            ...
            {content && content.json && (
              <div className={responsive ? "mt-4" : "ml-4 mt-5"}>
                <div className="text-xs">
                  <span
                    className={
                      responsive ? "whitespace-pre-wrap" : "whitespace-pre"
                    }
                  >
                    <div
                      className="border-[1px]"
                      dangerouslySetInnerHTML={{
                        __html: colorJson(content.json),
                      }}
                    />
                  </span>
                </div>
